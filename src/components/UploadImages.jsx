    ...
    widgetRef.current = cloudinaryRef.current.createUploadWidget(
      ...
      (error, result) => {
        if (result.event === "success") {
          setImageList((prev) => [...prev, { url: result.info.secure_url, publicId: result.info.public_id }]);
        }
      }
    );
  }, []);
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
        {imageList.map((item, index) => (
          <div key={index} className="relative">
            <IoMdCloseCircle
              ...
              className="absolute top-1 right-0.2 text-red-500 cursor-pointer text-xl"
            />
            <img
        ...
        <button
          ...
          className={`border rounded-xl border-dotted border-blue-500 bg-blue-100 p-10 cursor-pointer ${
            loader ? "opacity-50 cursor-not-allowed" : "hover:shadow-sm"
          }`}
        >
